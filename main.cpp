...
int main(int argc, char* argv[]) {
	...
	try {
		po::options_description desc("Opciones de simulacion y consulta");
		desc.add_options()
			...
			("inclinacion", po::value<double>(), "Inclinacion (i) en grados")
			...
			("anomalia-media", po::value<double>(), "Anomalia media inicial (M₀) en grados")
			("diametro", po::value<double>(), "Diametro del asteroide en metros")
			...
			("calculos-extendidos", po::bool_switch()->default_value(false), "Incluye calculos de efectos atmosfericos y destruccion")
		...
		if (vm.count("asteroide-id")) {
			...
			throw std::runtime_error("Debe proporcionar un --asteroide-id o los parametros orbitales manuales.");
		}
		...
		if (res_orbital.hay_impacto) {
			...
			if (vm["generar-resumen"].as<bool>()) {
				...
				prompt << "Actua como un experto en comunicacion de riesgos planetarios. "
					<< "Genera un resumen ejecutivo claro y conciso (maximo 150 palabras) sobre el siguiente escenario de impacto de asteroide. "
					<< "No uses lenguaje tecnico excesivo. Los datos son: "
					<< "Fecha de posible impacto/aproximacion: " << res_orbital.dia_impacto << "/"
					<< res_orbital.mes_impacto << "/" << res_orbital.anio_impacto << ". "
					<< "Distancia minima a la Tierra: " << std::fixed << std::setprecision(0)
				...
				if (res_orbital.hay_impacto) {
					prompt << "Lugar de impacto: Latitud " << std::setprecision(2)
						...
						<< "Energia liberada: " << std::setprecision(2)
						<< res_fisico.energia_cinetica_megatones << " megatones de TNT. "
						<< "Diametro del crater resultante: " << std::setprecision(1)
						<< res_fisico.diametro_crater_km << " kilometros. "
						<< "Magnitud sismica generada: " << std::setprecision(1)
			...
			output["nota"] = "No se detecto impacto directo. Se reporta la aproximacion mas cercana.";
		}
		std::cout << output.dump(4) << std::endl;
	}
